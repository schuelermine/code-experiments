class SlotsNamespaceMeta(type):
    def __call__(self, name, *args):
        def __init__(self, **kwargs):
            for arg in args:
                ...
                try:
                    ...
                    raise TypeError(
                        f"{arg!r} is a required keyword argument for {name}()"
                    ) from None
            ...
            if kwargs:
                raise TypeError(
                    f"{next(iter(kwargs))!r} is an invalid keyword argument for {name}()"
                )
        ...
        def __repr__(self):
            return (
                f"{name}("
                + ", ".join(f"{arg}={getattr(self, arg)!r}" for arg in args)
                + ")"
            )

        __repr__.__qualname__ = f"{name}.__repr__"
        namespace = {
            "__slots__": tuple(args),
            "__init__": __init__,
            "__repr__": __repr__,
        }
