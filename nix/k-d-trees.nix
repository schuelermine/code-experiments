...
in rec {
  get-k-d-tree-from-point-set = { dimensions ? maximum-of-list point-set
    , point-set ? [ ], cut-direction ? 0, default-coordinate ? 0 }:
    if length point-set != 0 then
      let
        get-coordinate = point:
          if length point >= cut-direction then
            elemAt point cut-direction
          else
            default-coordinate;
        ...
        partitioned-points =
          partition (point: get-coordinate point > cut-position)
          remaining-points;
        next-cut-direction =
          if cut-direction + 1 == dimensions then 0 else cut-direction + 1;
      in {
        _type = [ "k-d-tree" "node" ];
        point = median-point;
  ...
  get-closest-point-using-k-d-tree =
    { k-d-tree, target-point ? replicate k-d-tree.dimensions 0 }:
    null;
}
