...
def mk_decorator(*args):
    ...
    def _mk_decorator(dec, key):
        ...
        if isinstance(key, int):
            ...
            if key not in params:
                raise ValueError(f"Argument to mk_decorator() does not take argument number {key}")
        elif isinstance(key, str):
            def modify_args(args, kwargs, fun):
                kwargs[key] = fun
                return args, kwargs
            if key not in params:
                raise ValueError(f"Argument to mk_decorator() does not take argument {key}")
        else:
            raise TypeError("Argument to mk_decorator() must be an instance of int or str")
        @wraps(dec)
    ...
    if len(args) != 1:
        raise TypeError(f"mk_decorator() takes exactly one argument ({len(args)} given)")
    x = args[0]
    if callable(x):
