localFlake: rec {
    ...
    oldC = (nixos.lib.nixosSystem { system = builtins.currentSystem; modules = [ ~/old-config/configuration.nix ]; });
    ...
    u = with builtins; rec {
        cmpSets = v1: v2:
            if typeOf v1 != "set" || typeOf v2 != "set"
            then {
    ...
    filterAttrs = with builtins; p: s:
        foldl' (s1: s2: s1 // s2) { } (map (k: let v = s.${k}; in if p v then { ${k} = v; } else { }) (attrNames s));

    b = t: filterAttrs (s: let attempt = builtins.tryEval (s.fullyEqual or true); in if attempt.success then attempt.value else t) cmp;
